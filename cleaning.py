...
def clean_company_field(series: pd.Series) -> pd.Series:
    ...
    def clean_single_company(value):
        ...
        # Remove redundant descriptive suffixes
        redundant_patterns = [
...
def clean_dataframe(
    ...
    if region_mapping is not None:
        ...
        if "location" in cleaned.columns and "location" in region_mapping.columns:
            location_groups: dict[str, pd.DataFrame] = {
                str(loc): grp for loc, grp in region_mapping.groupby("location")
            }
    ...
    if {"geo_lat", "geo_lon"}.issubset(cleaned.columns):
        coord_mask = (
            cleaned["region"].isna()
            & (
                (cleaned["geo_lat"].notna() & cleaned["geo_lon"].notna()) |
                (cleaned["geo_lat"].notna() & cleaned["geo_lon"].isna()) |
                (cleaned["geo_lat"].isna() & cleaned["geo_lon"].notna())
            )
        )
        if coord_mask.any():
